...
// Сколько непустых операторов в тексте — по правилам своей СУБД (без dialect — наибольшее по всем).
// «;» в строках, комментариях и $$…$$ не считается; пустой хвост после последнего «;» — тоже.
function sqlStatementCount(sql, dialect) {
  return Math.max(...lexFor(dialect).map((lex) => stripSqlLiterals(sql, lex).split(';').filter((p) => p.trim()).length));
}
...
module.exports = { isReadOnlySql, sqlStatementCount, stripSqlLiterals, DESTRUCTIVE, sqlSegments, splitSqlStatements, findSqlParams, substituteSqlParams };
