...
function ensureProc(settings) {
  ...
  const gone = (msg) => {
    proc = null; ready = false;
    for (const [, w] of waiting) { clearTimeout(w.timer); w.resolve({ ok: false, error: msg }); }
    waiting.clear();
  };
  child.on('exit', (code, signal) => {
    if (proc !== child) return;   // этот уже остановлен через stop(): его ожидания разобраны там
    const wasReady = ready;
    gone('движок озвучки завершился');
    if (code !== 0 && code !== null) {
  ...
  child.on('error', (e) => {
    log('error', 'tts', 'spawn: ' + e.message);
    if (proc !== child) return;
    lastError = e.message;
    // Процесс не запустился вовсе (ENOENT/EACCES: интерпретатор удалили или сняли +x после проверки,
    // а ответ проверки закэширован) — 'exit' в этом случае не приходит, и мёртвый `proc` считался бы
    // живым: каждая фраза висела бы до таймаута в 2 минуты, а ensureProc так и не поднял бы новый.
    if (child.pid === undefined) { probeCache.delete(d.python); gone(e.message); }
  });
  // Запись в трубу уже умершего процесса (упал посреди фразы, а exit ещё не разобран) даёт EPIPE;
  // без слушателя это uncaughtException в main. Сам уход процесса разбирает 'exit'.
  child.stdin.on('error', (e) => log('warn', 'tts', 'stdin: ' + e.message));
  armIdle();
  return { ok: true };
}
...
function cacheKey(text, voice, rate, model) {
  return crypto.createHash('sha1').update([path.basename(model || ''), voice, rate, text].join('\0')).digest('hex');
}
// Чистку кэша запускаем не после каждой фразы (это readdir + stat на каждый файл), а изредка:
// потолок мягкий, а на длинном тексте фраз сотни. Первая синтезированная фраза запуска чистит
// сразу: счётчик живёт только в памяти, и кто озвучивает меньше 40 фраз за сеанс, иначе не
// доходил бы до чистки никогда — кэш рос без предела от запуска к запуску.
const PRUNE_EVERY = 40;
let sinceLastPrune = PRUNE_EVERY - 1;
...
function downloadModel(onProgress) {
  return new Promise((resolve) => {
    ...
    const start = (url, hops) => {
      ...
      // https.get бросает синхронно на чужом протоколе и битом адресе: без try исключение улетало
      // из колбэка ответа в uncaughtException main, промис не разрешался никогда, а `downloading`
      // так и оставался true — кнопка «Скачать» отвечала «уже идёт» до перезапуска редактора.
      let req;
      try { req = (url.startsWith('http://') ? http : https).get(url); } catch (e) { fail(e.message); return; }
      req.on('response', (res) => {
        const code = res.statusCode || 0;
        if (code >= 300 && code < 400 && res.headers.location) {
          res.resume();
          let next;
          try { next = new URL(res.headers.location, url); } catch (_) { fail('битый адрес перенаправления'); return; }
          // Модель — torch.package (pickle): при загрузке она исполняет код. С https на открытый
          // http (и тем более на другой протокол) не переходим — подмена по дороге стала бы
          // выполнением чужого кода. http→http остаётся для явного LITE_TTS_MODEL_URL (зеркало, тест).
          if (next.protocol !== 'https:' && !(next.protocol === 'http:' && url.startsWith('http://'))) {
            fail(`перенаправление на незащищённый адрес отклонено: ${next.protocol}`);
            return;
          }
          start(next.href, hops + 1);
          return;
        }
