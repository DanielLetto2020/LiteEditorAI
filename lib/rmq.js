...
async function api(c, method, path, body) {
  ...
  try {
    try {
      ...
      throw new Error(e && e.name === 'AbortError' ? 'Таймаут запроса (15с) — сервер не отвечает' : 'Нет соединения с ' + base + ' (' + String(e.message || e) + ')', { cause: e });
    }
    ...
    // Таймаут держим и на чтение тела: заголовки могли прийти, а тело повиснуть (прокси,
    // перегруженный брокер) — тогда полл секции стоял до bodyTimeout undici (5 минут).
    try { text = await res.text(); } catch (e) {
      throw new Error(e && e.name === 'AbortError' ? 'Таймаут запроса (15с) — сервер не отвечает' : 'Обрыв ответа от ' + base + ' (' + String(e.message || e) + ')', { cause: e });
    }
...
// Имя (vhost/очередь/exchange/подключение) → сегмент пути. «.» и «..» encodeURIComponent не трогает,
// а fetch (WHATWG URL) схлопывает их (и %2E) как точки пути: purge очереди «.» уходил в
// DELETE /api/queues/%2F/contents — удаление ЧУЖОЙ очереди «contents». Адресовать такие имена
// через URL нельзя — отказываем, а не бьём мимо.
function seg(name) {
  const s = String(name);
  if (s === '.' || s === '..') throw new Error(`Имя «${s}» нельзя адресовать через management API`);
  return encodeURIComponent(s);
}
const vh = (vhost) => seg(vhost || '/');
...
const tails = new Map();     // streamId -> { close, sender }
// Один 'destroyed'-listener на ОКНО (как в lib/storage.js), а не на каждый запуск тейла: иначе
// замыкания копились до закрытия окна, а с 11-го запуска сыпался MaxListenersExceededWarning.
const wiredSenders = new WeakSet();
function wireSender(sender) {
  if (wiredSenders.has(sender)) return;
  wiredSenders.add(sender);
  sender.once('destroyed', () => {
    for (const [sid, t] of [...tails]) if (t.sender === sender) { tails.delete(sid); t.close(); }
  });
}
...
async function openTail(c, { vhost, exchange, routingKey }, onMsg, onClose) {
  ...
  // Логин/пароль — через credentials, а не в userinfo URL: amqplib раскодирует его устаревшим
  // unescape() (Latin-1), и не-ASCII пароль превращался в кракозябры → ACCESS_REFUSED у тейла
  // при том, что management API с тем же профилем работал.
  const url = 'amqp://' + (c.host || '127.0.0.1') + ':' + (+c.amqpPort || 5672) + '/' + encodeURIComponent(vhost || '/');
  // timeout: без него порт, принявший TCP, но молчащий (не AMQP / файрвол), вешал запуск тейла
  // навсегда — «Слушаю…» без ошибки и сокет в main до выхода. amqplib снимает его после рукопожатия.
  const conn = await amqplib.connect(url, { credentials: amqplib.credentials.plain(c.user || 'guest', dec(c.passEnc)), timeout: 10000 });
  conn.on('error', () => {}); // обрыв без слушателя = uncaught exception в main
  // close() у amqplib — промис: на уже закрытом соединении/канале он отклоняется («Connection
  // closed»), и try/catch этого не ловит — обрыв во время настройки давал unhandledRejection.
  let ch;
  try {
    ...
    await ch.consume(q.queue, (msg) => {
      // null — брокер отменил консюмер (временную очередь удалили, хоть из вкладки «Очереди»):
      // сообщений больше не будет, а соединение живо и тейл «слушал» бы вечно. Закрываем —
      // 'close' ниже сообщит окну, что поток закрыт.
      if (!msg) { try { conn.close().catch(() => {}); } catch (_) {} return; }
  ...
  } catch (e) { try { conn.close().catch(() => {}); } catch (_) {} throw e; }
  conn.on('close', () => { if (onClose) onClose(); });
  return { close: () => { try { ch.close().catch(() => {}); } catch (_) {} try { conn.close().catch(() => {}); } catch (_) {} } };
}

// ---------------------------------------------------------------- IPC
/** @param {{ ipcMain: import('electron').IpcMain, safeStorage?: any, getConnections?: any, setConnections?: any }} deps */
function registerRmqIpc({ ipcMain, safeStorage, getConnections, setConnections }) {
  ...
  ipcMain.handle('rmq:overview', dataCall(async (c, /** @type {any} */ { age } = {}) => {
  ...
  ipcMain.handle('rmq:peek', dataCall(async (c, { vhost, queue, count }) => {
    const msgs = await api(c, 'POST', `/api/queues/${vh(vhost)}/${seg(queue)}/get`,
  ...
  ipcMain.handle('rmq:publish', actionCall(async (c, { vhost, exchange, routingKey, payload, properties }) => {
    const ex = exchange ? seg(exchange) : 'amq.default';
  ...
  ipcMain.handle('rmq:purge', actionCall(async (c, { vhost, queue }) =>
    { await api(c, 'DELETE', `/api/queues/${vh(vhost)}/${seg(queue)}/contents`); }));
  ipcMain.handle('rmq:deleteQueue', actionCall(async (c, { vhost, queue }) =>
    { await api(c, 'DELETE', `/api/queues/${vh(vhost)}/${seg(queue)}`); }));
  ipcMain.handle('rmq:killConnection', actionCall(async (c, { name }) =>
    { await api(c, 'DELETE', '/api/connections/' + seg(name)); }));
  ...
  ipcMain.handle('rmq:tailStart', async (e, { id, vhost, exchange, routingKey, streamId } = {}) => {
    ...
    try {
      ...
      const t = await openTail(c, { vhost, exchange, routingKey },
        (m) => sendSafe(sender, 'rmq:tailData', { streamId, ...m }),
        () => { if (tails.delete(streamId)) sendSafe(sender, 'rmq:tailExit', { streamId }); });
      // Окно закрыли, пока шёл AMQP-коннект: 'destroyed' уже отгремел, и подписка ниже
      // не сработала бы никогда — соединение жило бы до выхода из редактора.
      if (sender.isDestroyed()) { t.close(); return { ok: false, error: 'Окно закрыто' }; }
      tails.set(streamId, { close: t.close, sender });
      wireSender(sender);
      return { ok: true };
