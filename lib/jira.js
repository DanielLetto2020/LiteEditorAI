...
async function readResponse(a, res, url) {
  ...
  // 204 No Content — успех без тела (так Jira отвечает на POST /transitions), Content-Type у него
  // нет. Страницей входа SSO он быть не может, а ветка ниже объявляла успешный переход ошибкой.
...
async function api(a, method, apiPath, body, opts = {}) {
  ...
  // До fetch: «не задан токен» — не сетевая ошибка, внутри try её заворачивало в «Нет соединения…».
  const auth = opts.anon ? '' : authHeader(a);
  ...
  try {
    ...
    // Сам «fetch failed» ничего не говорит — причина в cause (ECONNREFUSED, ENOTFOUND, сертификат
    // внутреннего УЦ вроде SELF_SIGNED_CERT_IN_CHAIN): без неё недоверенный сертификат выглядел как «поднимите VPN».
    const c = e && e.cause;
    const why = c ? String(c.code || c.message || '') : '';
    throw new Error('Нет соединения с ' + base + ' (' + String(e.message || e) + (why ? ': ' + why : '') + '). Если Jira за VPN — проверьте, что он поднят.', { cause: e });
  }
  // Таймер снимаем только после чтения ТЕЛА: сервер/прокси, отдавший заголовки и замолчавший,
  // иначе вешал вызов навсегда — fetch резолвится уже на заголовках.
  try { return await readResponse(a, res, url); }
  catch (e) {
    if (e && e.name === 'AbortError') throw new Error('Таймаут запроса (' + (TIMEOUT_MS / 1000) + ' с) — Jira не отвечает', { cause: e });
    throw e;
...
async function searchPage(a, jql, p) {
  ...
  let r;
  try {
    r = await api(a, 'GET', '/rest/api/2/search?' + q({
      jql, fields: fields || LIST_FIELDS, maxResults: limit, startAt: startAt || 0,
    }));
  } catch (e) {
    // Классического /search здесь нет (Cloud отвечает 410 Gone) — значит, 400 от /search/jql был
    // про сам запрос (опечатка в своём JQL), и показать нужно его, а не «API has been removed».
    if (jqlErr && /HTTP (404|410)/.test(String(e.message || e))) throw jqlErr;
    throw e;
  }
...
async function searchAll(a, jql, opts = {}) {
  ...
  for (;;) {
    ...
    // Пустая страница при «есть ещё» — курсор/startAt не сдвинулся, и дальше запрашивалась бы
    // та же страница по кругу: тысячи запросов в секунду к Jira и вечный «Загружаю задачи…».
    if (!r.issues.length) return { issues: out, total: total ?? out.length, truncated: (total || 0) > out.length };
...
async function fieldsInfo(a) {
  ...
  // Провал не кэшируем: кэш живёт до перезапуска приложения, и Jira, открытая до подъёма VPN,
  // иначе так и оставалась без спринтов, бейджа типа и имён полей (сырые customfield_NNNNN).
  if (got) fieldInfo.set(a.id, info);
  return info;
}
...
function registerJiraIpc({ ipcMain, safeStorage, getAccounts, setAccounts, storeDir }) {
  ...
  ipcMain.handle('jira:save', (_e, { account } = {}) => {
    ...
    delete rec.token;
    rec.host = normalizeHost(rec.host);   // ссылка на задачу/дашборд из адресной строки → корень REST API
    if (!rec.host) return { ok: false, error: 'не задан адрес Jira' };
  ...
  ipcMain.handle('jira:delete', (_e, { id } = {}) => {
    ...
    fieldInfo.delete(id);   // карта полей удалённой Jira (сотни имён) иначе жила до перезапуска
    return { ok: true };
  });
  ...
  ipcMain.handle('jira:test', async (_e, { account } = {}) => {
    ...
    cfg.host = normalizeHost(cfg.host);   // как при сохранении: проверяем тот адрес, который и запишется
    try {
  ...
  ipcMain.handle('jira:issue', call(async (a, { key }) => {
    ...
    return {
      issue: {
        // typeIds — как в списке (listFieldsFor): без них бейдж типа в карточке расходился с бейджем
        // строки, когда явное поле в задаче пустое, а другое поле «типа» заполнено.
        ...slimIssue(a, it, { sprintId: fi.sprintId, typeFieldId: fi.typeFieldId, typeIds: fi.typeIds }),
  ...
  ipcMain.handle('jira:reconSave', async (_e, { report } = {}) => {
    try {
      ...
      // id приходит из рендерера вместе с отчётом (и из стора, куда его может положить импорт
      // настроек) — «/» и «..» в нём увели бы файл из каталога jira/.
      const accId = String((report.account && report.account.id) || 'acc').replace(/[^\w-]/g, '_');
