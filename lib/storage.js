...
function registerStorageIpc({ ipcMain, safeStorage, dialog, getConnections, setConnections }) {
  ...
  ipcMain.handle('st:stage', dataCall(async (c, { bucket, key }) => {
    ...
    // mkdtemp прямо в tmpdir (0700, имя из CSPRNG): общий родитель liteeditor-storage/ с recursive+0700
    // получал бы 0700 от первого пользователя машины, и у остальных «Открыть в вивере» падал с EACCES.
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'liteeditor-storage-'));
