...
//
// main.js: registerStorageIpc({ ipcMain, safeStorage, dialog, getConnections, setConnections, stageDir }).
...
let _safe = null, _get = null, _set = null, _stageDir = null;
...
function publicConn(c) {
  ...
  const a = adapterByName(c.adapter || 's3');
...
// Адаптер грузится при первом обращении: storage-s3 тянет @aws-sdk (~50 мс синхронной загрузки),
// а на старте редактора он не нужен.
const ADAPTER_LOADERS = { s3: () => require('./storage-s3') };
const ADAPTERS = {};
function adapterByName(name) {
  if (!ADAPTERS[name] && ADAPTER_LOADERS[name]) ADAPTERS[name] = ADAPTER_LOADERS[name]();
...
// Эвикция: клиент СРАЗУ снимается с кэша — следующий запрос получит свежий, с новыми настройками;
// а destroy() откладывается, пока под старым клиентом идут передачи (он оборвал бы их посреди
// потока: идёт загрузка → пользователь пересохранил подключение). Раньше на это время в кэше
// оставался старый клиент, и листинг/чтение до конца передачи шли на старый endpoint/ключи.
const retired = new Set();   // клиенты, снятые с кэша, но ещё занятые передачами
const opClients = new Map(); // opId -> client — под каким клиентом идёт живая передача
function reapRetired() {
  const busy = new Set(opClients.values());
  for (const cl of retired) if (!busy.has(cl)) { retired.delete(cl); try { cl.destroy && cl.destroy(); } catch (_) {} }
}
function evict(id) {
  const h = clients.get(id);
  if (!h) return;
  clients.delete(id);
  retired.add(h.client);
  reapRetired();
}
...
/** @param {{ ipcMain: import('electron').IpcMain, safeStorage?: any, dialog?: any, getConnections?: any, setConnections?: any, stageDir?: () => string }} deps */
function registerStorageIpc({ ipcMain, safeStorage, dialog, getConnections, setConnections, stageDir }) {
  _safe = safeStorage; _get = getConnections; _set = setConnections; _stageDir = stageDir || null;
  ...
  // Какие из ключей уже есть в бакете (HEAD по каждому). Нужна загрузке: листинг в окне — первая страница
  // (до 1000 ключей), и одноимённый объект со следующей страницы перезаписывался без вопроса.
  ipcMain.handle('st:exists', dataCall(async (c, { bucket, keys }) => {
    const a = adapterOf(c), client = clientFor(c), found = [];
    for (const key of (Array.isArray(keys) ? keys : []).slice(0, 500)) {
      try { await a.stat(client, { bucket, key: String(key) }); found.push(String(key)); }
  ...
  // Пустой префикс = ВЕСЬ бакет (до 20000 объектов без единого вопроса). UI шлёт только префикс
  // папки («x/»), но граница безопасности — бэкенд (как guardWrite): такой запрос не исполняем.
  ipcMain.handle('st:removePrefix', writeCall(async (c, { bucket, prefix }) => {
    if (!prefix || typeof prefix !== 'string') throw new Error('Не указан префикс папки — удаление всего бакета недоступно');
    return adapterOf(c).removePrefix(clientFor(c), { bucket, prefix });
  }));
  ...
  function finishOp(opId) {
    ...
    if (!t) return null;
    ops.delete(opId);
    opClients.delete(opId);
    reapRetired(); // отложенная эвикция: снятый с кэша клиент уничтожается после своей последней передачи
    return t;
  }
  function runOp(sender, opId, key, client, task) {
    ops.set(opId, { abort: task.abort, sender });
    opClients.set(opId, client);
    wireSender(sender);
    task.done
  ...
  ipcMain.handle('st:upload', (e, { id, bucket, key, filePath, opId } = {}) => {
    try {
      ...
      const client = clientFor(c);
      const task = adapterOf(c).upload(client, { bucket, key, filePath, onProgress: progressSender(e.sender, opId, 'upload', key) });
      runOp(e.sender, opId, key, client, task);
      return { ok: true };
  ...
  ipcMain.handle('st:download', (e, { id, bucket, key, destPath, opId } = {}) => {
    try {
      ...
      const client = clientFor(c);
      const task = adapterOf(c).download(client, { bucket, key, destPath, onProgress: progressSender(e.sender, opId, 'download', key) });
      runOp(e.sender, opId, key, client, task);
      return { ok: true };
    } catch (e2) { return { ok: false, error: String(e2.message || e2) }; }
  });
  ...
  ipcMain.handle('st:downloadPrefix', (e, { id, bucket, prefix, destDir, opId } = {}) => {
    try {
      ...
      const client = clientFor(c);
      const task = downloadPrefixTask(adapterOf(c), client, { bucket, prefix, destDir, onProgress: progressSender(e.sender, opId, 'download', prefix) });
      runOp(e.sender, opId, prefix, client, task);
      return { ok: true };
    } catch (e2) { return { ok: false, error: String(e2.message || e2) }; }
  });

  // «Открыть в вивере редактора»: объект → временный файл на диске (read-only копия).
  // Объект бывает приватным (выгрузки, конфиги), а /tmp общий → каталог из CSPRNG 0700, файл 0600
  // (как stageTextForViewer в main.js); раньше — общий liteeditor-storage/ с предсказуемыми
  // именами и правами по umask, читаемый соседним пользователем.
  ...
  ipcMain.handle('st:stage', dataCall(async (c, { bucket, key }) => {
    ...
    // mkdtemp (0700, имя из CSPRNG): общий родитель liteeditor-storage/ с recursive+0700 получал бы 0700
    // от первого пользователя машины, и у остальных «Открыть в вивере» падал с EACCES. Корень — общий
    // с остальными копиями для вивера (main.js viewStageDir): его сносят на выходе и подбирают после
    // краха. Прямо в tmpdir копии объектов (до 12 МБ каждая) копились до перезагрузки, в %TEMP% — навсегда.
    const dir = fs.mkdtempSync(path.join(_stageDir ? _stageDir() : os.tmpdir(), 'liteeditor-storage-'));
    const safe = safeRelSegments(String(key).split('/').pop()).join('_'); // «..»/пусто/NUL → безопасное имя
