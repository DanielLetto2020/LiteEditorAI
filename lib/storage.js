...
function registerStorageIpc({ ipcMain, safeStorage, dialog, getConnections, setConnections }) {
  ...
  function runOp(sender, opId, key, connId, task) {
    ...
    wireSender(sender);
    task.done
      .then(() => { if (finishOp(opId) && !sender.isDestroyed()) sender.send('st:done', { opId, key }); })
      .catch((e) => { if (finishOp(opId) && !sender.isDestroyed()) sender.send('st:opErr', { opId, key, error: String(e.message || e) }); });
  }
  ...
  function downloadPrefixTask(a, client, { bucket, prefix, destDir, onProgress }) {
    ...
    const done = (async () => {
      ...
      for (const o of items) {
        ...
        const dest = path.join(destDir, ...rel.split('/').filter(Boolean));
