...
function humanErr(e) {
  ...
  if (name === 'AccessControlListNotSupported' || name === 'NotImplemented') return 'ACL_UNSUPPORTED';
  if (name === 'AbortError' || /aborted/i.test(msg)) return 'Операция отменена';
  return (name ? name + ': ' : '') + msg.slice(0, 200);
}
const wrap = (e) => new Error(humanErr(e));
...
async function aclGet(client, { bucket, key }) {
  try {
    ...
    if (humanErr(e) === 'ACL_UNSUPPORTED') return { public: false, supported: false };
    throw wrap(e);
  }
}
async function aclSet(client, { bucket, key, public: pub }) {
  try {
    ...
    if (humanErr(e) === 'ACL_UNSUPPORTED') {
      const err = new Error('На этом хранилище ACL объектов отключены — публичность задаётся политикой бакета');
...
function download(client, { bucket, key, destPath, onProgress }) {
  ...
  const done = (async () => {
    const r = await client.send(new GetObjectCommand({ Bucket: bucket, Key: key }), { abortSignal: ctl.signal });
    ...
    try {
      ...
      try { out.destroy(); fs.unlinkSync(destPath); } catch (_) {} // недокачанный файл не оставляем
      throw wrap(e);
    }
  })();
...
async function remove(client, { bucket, keys }) {
  try {
    ...
    for (let i = 0; i < keys.length; i += 1000) {
      ...
      if (r.Errors && r.Errors.length) throw new Error(`Удалено ${deleted}, ошибка на «${r.Errors[0].Key}»: ${r.Errors[0].Message}`);
    }
    return { deleted };
  } catch (e) { throw wrap(e); }
}

// Удалить всё под префиксом («папку»): итерируем листинг и удаляем пачками. Лимит — защита от
// случайного сноса гигантского бакета одной кнопкой.
const REMOVE_PREFIX_LIMIT = 20000;
async function removePrefix(client, { bucket, prefix }) {
  try {
    let token = null, deleted = 0;
    for (;;) {
      ...
      if (keys.length) {
        const d = await remove(client, { bucket, keys });
        deleted += d.deleted;
      }
      if (deleted > REMOVE_PREFIX_LIMIT) throw new Error(`Прервано: под префиксом больше ${REMOVE_PREFIX_LIMIT} объектов (удалено ${deleted})`);
...
module.exports = {
  makeClient, humanErr, publicUrl, encodeKey, mimeOf,
  test, listBuckets, ls, stat, readRange, readObject, presign, aclGet, aclSet,
