...
function download(client, { bucket, key, destPath, onProgress }) {
  ...
  const done = (async () => {
    ...
    try {
      ...
      tmp = path.join(path.dirname(destPath), '.lite-' + crypto.randomBytes(6).toString('hex') + '.part');
      ...
      // файл мог ещё открываться (ранняя отмена) — ждём закрытия, иначе пустой .part остался бы на диске
