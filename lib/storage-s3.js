...
const {
  ...
  GetObjectAclCommand, PutObjectAclCommand, UploadPartCommand,
...
function humanErr(e) {
  ...
  // Системные ошибки Node несут суть в e.code, а name у них 'Error': обрыв сокета посреди тела
  // ответа — { code:'ECONNRESET', message:'aborted' } — раньше попадал в «Операция отменена»,
  // а ветка ENOENT (нет локального файла для загрузки) не срабатывала никогда.
  const code = String((e && e.code) || '');
  if (/ENOTFOUND|EAI_AGAIN/.test(msg + ' ' + code)) return 'Хост не найден (DNS): ' + msg.slice(0, 140);
  if (/ECONNREFUSED|EHOSTUNREACH|ETIMEDOUT|ECONNRESET|socket hang up/i.test(msg + ' ' + code)) return 'Нет соединения с хранилищем: ' + msg.slice(0, 140);
  if (name === 'ENOENT' || code === 'ENOENT') return 'Локальный файл не найден: ' + msg.slice(0, 160);
...
const wrap = (e) => {
  if (e && e.__human) return e;
  const err = /** @type {Error & { __human?: boolean, notFound?: boolean }} */ (new Error(humanErr(e)));
  err.__human = true;
  // «нет такого объекта» — не сбой, а ответ (проверка существования перед загрузкой, st:exists)
  err.notFound = ['NoSuchKey', 'NotFound'].includes((e && (e.name || e.Code)) || '') || !!(e && e.$metadata && e.$metadata.httpStatusCode === 404);
  return err;
};
const hErr = (msg) => { const err = /** @type {Error & { __human?: boolean, aclUnsupported?: boolean }} */ (new Error(msg)); err.__human = true; return err; };
...
function upload(client, { bucket, key, filePath, onProgress }) {
  ...
  // Upload.abort() не прерывает запросы, которые уже в полёте: одиночный PUT (файл ≤ partSize)
  // докачивался и ЗАМЕНЯЛ объект, хотя в UI «отменено», а части multipart доливались до конца.
  // Подмешиваем сигнал отмены в PutObject/UploadPart; AbortMultipartUpload (уборка частей после
  // отмены) уходит без него — иначе он сам оборвался бы и части остались бы висеть в бакете.
  const ctl = new AbortController();
  const abortable = new Proxy(client, {
    get: (t, p) => (p !== 'send' ? t[p]
      : (cmd, opts) => t.send(cmd, (cmd instanceof PutObjectCommand || cmd instanceof UploadPartCommand) ? { ...(opts || {}), abortSignal: ctl.signal } : opts)),
  });
  const up = new Upload({
    client: abortable, abortController: ctl,
...
// Пишем во временного соседа и переименовываем поверх destPath только после полной докачки:
// раньше catch делал unlink(destPath) на ЛЮБОЙ ошибке — 404/403/обрыв сети/отмена ещё до записи
// удаляли уже лежавший там файл человека, а обрыв посреди потока оставлял его обрезанным.
function download(client, { bucket, key, destPath, onProgress }) {
  const ctl = new AbortController();
  const done = (async () => {
    let r, out, tmp, made = false;
    try {
      ...
      if (ctl.signal.aborted) throw hErr('Операция отменена');
      // суффикс .lite-tmp — соглашение ядра (WRITE_TMP_SUFFIX в main.js): дерево и вотчер проекта
      // такого соседа не показывают и не шлют «файл изменился» на каждый записанный чанк
      tmp = path.join(path.dirname(destPath), '.lite-' + crypto.randomBytes(6).toString('hex') + '.lite-tmp');
      out = fs.createWriteStream(tmp, { flags: 'wx' });
      out.once('open', () => { made = true; }); // 'wx' не открылся — файл не наш, не удаляем
      let loaded = 0;
      await new Promise((resolve, reject) => {
        r.Body.on('data', (chunk) => { loaded += chunk.length; if (onProgress) onProgress(loaded, total); });
        r.Body.on('error', reject);
        out.on('error', reject);
        // ждём 'close' (не 'finish'): дескриптор закрыт — rename ниже не упрётся в открытый файл
        out.on('close', () => { if (out.writableFinished) resolve(null); });
      ...
      try { if (r && r.Body && r.Body.destroy) r.Body.destroy(); } catch (_) {} // сокет ответа не держим
      // файл мог ещё открываться (ранняя отмена) — ждём закрытия, иначе пустой временный файл остался бы на диске
      if (out) await new Promise((res) => { if (out.closed) res(null); else { out.once('close', () => res(null)); out.destroy(); } });
      if (made) { try { fs.unlinkSync(tmp); } catch (_) {} } // недокачанный файл не оставляем
      throw wrap(e);
    }
  })();
