...
function createIterflowApi(opts) {
  ...
  function httpError(res, data) {
    const e = /** @type {Error & { status?: number, code?: string }} */ (new Error((data && data.error) || ('HTTP ' + res.status)));
    e.status = res.status;
  ...
  async function authedCookie(method, p, body) {
    if (!session || !session.cookie) { const e = /** @type {Error & { web401?: boolean }} */ (new Error('Нет веб-сессии IterFlow — выйдите и войдите снова')); e.web401 = true; throw e; }
    const res = await fetch(base() + p, {
      method,
      headers: { 'Content-Type': 'application/json', 'Cookie': session.cookie },
      body: body !== undefined ? JSON.stringify(body) : undefined,
    });
    if (res.status === 401) { const e = /** @type {Error & { web401?: boolean }} */ (new Error('Веб-сессия IterFlow истекла — выйдите и войдите снова')); e.web401 = true; throw e; }
    const data = await parseBody(res);
    if (!res.ok) throw httpError(res, data);
    return data;
  }
  ...
  return {
    ...
    // action кодируется, как и прочие сегменты: из рендерера «../../…» иначе давал POST с веб-cookie
    // на любой маршрут /api/*, а не только на переходы стадий.
    async iterationStage(id, action, body) { return authedCookie('POST', '/api/iterations/' + encodeURIComponent(id) + '/' + encodeURIComponent(action), body); },
