...
// kafkajs грузится при первом обращении к Kafka, а не на старте редактора (~25 мс до создания окна).
let _kjs = null;
const kjs = () => _kjs || (_kjs = require('kafkajs'));
...
function kafkaFor(c) {
  ...
  const cfg = {
    clientId: 'liteeditor', brokers, logLevel: kjs().logLevel.NOTHING,
...
async function adminFor(c) {
  ...
  if (!ent) {
    ...
    admins.set(key, ent);
    // Мёртвый коннект не должен залипнуть в пуле. Удаляем только СВОЮ запись: поздний отказ
    // connect() записи, уже выброшенной dropAdmin (таймаут), иначе выкидывал новую — и её
    // подключившийся admin оставался открытым без владельца до выхода из редактора.
    const own = ent;
    ent.ready.catch(() => { if (admins.get(key) === own) admins.delete(key); });
  }
...
async function openTail(c, { topic }, onMsg, onClose) {
  ...
  try {
    await withTimeout(consumer.connect(), 8000, 'подключение консюмера');
    // CRASH kafkajs эмитит ДО того, как взведёт таймер авто-рестарта (restart:true), а disconnect()
    // гасит только уже взведённый таймер: cleanup прямо в обработчике оставлял зомби-консюмер,
    // который через retryTime сам переподключался к брокеру. Поэтому — на следующем тике.
    consumer.on(consumer.events.CRASH, () => { setImmediate(() => { cleanup(); if (onClose) onClose(); }); });
...
function registerKafkaIpc({ ipcMain, safeStorage, getConnections, setConnections }) {
  ...
  ipcMain.handle('kafka:test', async (_e, { conn } = {}) => {
    ...
    let admin = null;
    try {
      // kafkaFor бросает на пустом списке брокеров (поле из одних запятых) — это тоже ответ
      // {ok:false}, а не отказ invoke: кнопка «Тест» зависала на «Проверяю…».
      admin = kafkaFor(cfg).admin();
    ...
    finally { if (admin) admin.disconnect().catch(() => {}); }
  });
  ...
  ipcMain.handle('kafka:topics', dataCall(async (c, /** @type {any} */ { internal } = {}) => {
    const admin = await adminFor(c);
  ...
  ipcMain.handle('kafka:topicDetail', dataCall(async (c, { topic }) => {
    ...
    const [meta, offs, cfg] = await Promise.all([
      ...
      withTimeout(admin.describeConfigs({ resources: [{ type: kjs().ConfigResourceTypes.TOPIC, name: topic }], includeSynonyms: false }), 12000, 'конфиги').catch(() => null),
    ]);
  ...
  ipcMain.handle('kafka:setTopicConfig', actionCall(async (c, { topic, name, value }) => {
    ...
    await withTimeout(admin.alterConfigs({
      validateOnly: false,
      resources: [{ type: kjs().ConfigResourceTypes.TOPIC, name: topic, configEntries: [{ name, value: String(value) }] }],
  ...
  ipcMain.handle('kafka:groupDetail', dataCall(async (c, { groupId }) => {
    ...
    const members = (g.members || []).map((mb) => {
      let assign = '';
      try {
        const a = kjs().AssignerProtocol.MemberAssignment.decode(mb.memberAssignment);
  ...
  ipcMain.handle('kafka:tailStart', async (e, { id, topic, streamId } = {}) => {
    ...
    try {
      ...
      const t = await openTail(c, { topic },
        (m) => sendSafe(sender, 'kafka:tailData', { streamId, ...m }),
        () => { if (tails.delete(streamId)) sendSafe(sender, 'kafka:tailExit', { streamId }); });
      // Окно закрыли, пока консюмер подключался: 'destroyed' уже отгремел, и подписка ниже
      // не сработала бы никогда — стрим жил бы до выхода из редактора.
