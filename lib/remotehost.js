...
// Метаданные одного файла (шапка открытого файла): SFTP-stat; для FTP их даёт листинг каталога.
async function fsStat(c, filePath) {
  const b = await getBrowser(c);
  if (b.type !== 'sftp') return { ok: true, meta: null };
  const st = await sftpStat(b.sftp, filePath);
  const mode = Number.isFinite(st.mode) ? st.mode : null;
  return { ok: true, meta: {
    size: st.size || 0,
    perms: permsFromMode(mode),
    mode: modeOctal(mode),
    type: fileTypeChar(mode),
    owner: st.uid != null ? String(st.uid) : '',
    group: st.gid != null ? String(st.gid) : '',
    mtime: toMillis(st.mtime),
  } };
}
...
function registerRemoteIpc({ ipcMain, safeStorage, send, getConnections, setConnections, onSessionOpen }) {
  ...
  ipcMain.handle('rh:fsStat', async (_e, { id, path: p } = {}) => {
    const c = loadConns().find((x) => x.id === id); if (!c) return { ok: false, error: 'Подключение не найдено' };
    try { return await fsStat(c, p); } catch (e) { return { ok: false, error: String(e.message || e) }; }
  });
