...
// ssh2 грузится при первом подключении, а не на старте редактора (~25 мс до создания окна).
let _ssh2 = null;
const SshClientCtor = () => (_ssh2 || (_ssh2 = require('ssh2'))).Client;
const { permsFromMode, modeOctal, parseLongname, permsFromFtp, isDirMode, isLinkMode, fileTypeChar, toMillis } = require('./fsmeta');
...
// Только ключи, которые читаются БЕЗ пароля: зашифрованный ключ (обычная схема — ключ под паролем,
// разблокирован в ssh-agent) ssh2 не разбирает и бросает «Cannot parse privateKey» прямо в connect(),
// так что до агента дело не доходило и вход «без пароля» падал целиком.
function defaultIdentityFiles() {
  const dir = path.join(os.homedir(), '.ssh');
  const out = [];
  let parseKey = null;
  try { parseKey = (_ssh2 || (_ssh2 = require('ssh2'))).utils.parseKey; } catch (_) {}
  for (const f of ['id_ed25519', 'id_ecdsa', 'id_rsa', 'id_dsa']) {
    try {
      const p = path.join(dir, f);
      if (!fs.existsSync(p)) continue;
      const buf = fs.readFileSync(p);
      if (parseKey && parseKey(buf) instanceof Error) continue; // зашифрован или не ключ — этим займётся агент
      out.push(buf);
    } catch (_) {}
  }
  return out;
...
// Сессии, которые ещё подключаются (sessionId -> ssh2-клиент): закрыть вкладку/окно можно и во время
// коннекта, и без этой карты cleanup не находил сессию — она поднималась уже никому не нужной и жила
// до выхода из редактора.
const pendingSessions = new Map();
function cleanup(id) {
  const s = sessions.get(id);
  if (s) { try { s.stream.end(); } catch (_) {} try { s.ssh.end(); } catch (_) {} sessions.delete(id); if (s.unbind) s.unbind(); }
  const p = pendingSessions.get(id);
  if (p) { pendingSessions.delete(id); try { p.end(); } catch (_) {} }
}
...
function openSession(sessionId, c, cols, rows) {
  return new Promise((resolve) => {
    ...
    let settled = false;
    const finish = (res) => { if (!settled) { settled = true; if (pendingSessions.get(sessionId) === ssh) pendingSessions.delete(sessionId); resolve(res); } };
    pendingSessions.set(sessionId, ssh);
    // Сервер закрыл соединение до готовности без DISCONNECT-ошибки (ssh2 шлёт тогда только 'close') или
    // попытку отменил cleanup — иначе обещание не резолвилось никогда.
    ssh.on('close', () => { if (!settled) { _send('rh:exit', { id: sessionId }); finish({ error: 'Соединение закрыто' }); } });
    ssh.on('error', (e) => {
    ...
    ssh.on('ready', () => {
      ssh.shell({ term: 'xterm-256color', cols: cols || 80, rows: rows || 24 }, (err, stream) => {
        ...
        // сессию закрыли, пока открывался канал (cleanup уже снял её из pendingSessions) — не регистрируем
        if (settled || pendingSessions.get(sessionId) !== ssh) { try { stream.close(); } catch (_) {} try { ssh.end(); } catch (_) {} return finish({ error: 'Сессия закрыта' }); }
        sessions.set(sessionId, { ssh, stream, connId: c.id });
        // Декодер потока, а не toString на каждый пакет: многобайтный символ (кириллица), разрезанный
        // между пакетами SSH, превращался в «��».
        stream.setEncoding('utf8');
        if (stream.stderr) stream.stderr.setEncoding('utf8');
...
function decodeForView(buf) {
  ...
  // Не UTF-8 (windows-1251 на старом сервере): текст с «�» вместо букв, и сохранение залило бы его на
  // хост поверх исходного. Такой файл — как двоичный: смотреть нельзя, скачать можно.
  if (!isUtf8(buf)) return { ok: true, binary: true, notUtf8: true, size: buf.length };
...
function sftpConnect(c) {
  return new Promise((resolve, reject) => {
    ...
    ssh.on('error', fail);
    // сервер закрыл соединение до готовности без ошибки (ssh2 шлёт только 'close') — иначе висели бы
    // и этот запрос, и все следующие к профилю (они ждут то же обещание из pendingBrowsers)
    ssh.on('close', () => fail(new Error('Соединение закрыто')));
...
async function ftpConnect(c) {
  ...
  return { type: 'ftp', ftp: client, queue: Promise.resolve() };
}
// basic-ftp не умеет параллельных команд: задача, запущенная посреди другой, ЗАКРЫВАЕТ клиент с ошибкой
// (падают обе). А параллельные запросы здесь — норма: раскрыть каталог, пока грузится другой или
// открывается файл, заливка из вивера посреди листинга. Операции одного FTP-соединения — строго по очереди.
function ftpSerial(b, fn) {
  const run = b.queue.then(fn);
  b.queue = run.then(() => {}, () => {});
  return run;
}
// В карту кладём ОБЕЩАНИЕ подключения, а не готовое: между промахом кэша и первым await проходят
// другие запросы (браузер файлов легко шлёт листинг и чтение параллельно), и каждый открывал бы
// своё SFTP/FTP-соединение. Оставалось последнее, остальные утекали — closeBrowser знает только
// про то, что в карте.
const pendingBrowsers = new Map(); // connId -> Promise<browser>
async function getBrowser(c) {
  const cached = browsers.get(c.id);
  // FTP-клиент, закрытый сервером или ошибкой (basic-ftp держит флаг closed), не отдаём — переподключаемся
  if (cached && !(cached.ftp && cached.ftp.closed)) return cached;
  if (cached) closeBrowser(c.id);
  const inflight = pendingBrowsers.get(c.id);
  if (inflight) return inflight;
  const p = (async () => {
    const b = (c.type === 'ftp') ? await ftpConnect(c) : await sftpConnect(c);
    browsers.set(c.id, b);
    // SFTP умер (сеть, keepalive, сервер) — сразу убрать из карты: иначе мёртвый канал отдавался дальше,
    // а запрос в закрытый канал ssh2 не отвечает никогда — открытие файла и заливка из вивера висели.
    if (b.ssh) {
      const drop = () => { if (browsers.get(c.id) === b) { browsers.delete(c.id); try { b.ssh.end(); } catch (_) {} } };
      b.ssh.on('close', drop);
      b.sftp.on('close', drop);
    }
    return b;
  })();
  pendingBrowsers.set(c.id, p);
  try { return await p; } finally { pendingBrowsers.delete(c.id); }
}
...
const sftpReadlink = (s, p) => new Promise((res) => s.readlink(p, (e, target) => res(e ? '' : target)));
const sftpReadFile = (s, p) => new Promise((res, rej) => s.readFile(p, (e, buf) => e ? rej(e) : res(buf)));
const sftpWriteFile = (s, p, buf) => /** @type {Promise<void>} */ (new Promise((res, rej) => s.writeFile(p, buf, (e) => e ? rej(e) : res())));
const sftpFastGet = (s, remote, local) => /** @type {Promise<void>} */ (new Promise((res, rej) => s.fastGet(remote, local, (e) => e ? rej(e) : res())));

// Максимум симлинков в каталоге, для которых доразрешаем цель (каждый — ещё два запроса к серверу;
// на каталогах вроде /usr/bin их сотни, и полное разрешение превращает листинг в минуту ожидания).
const MAX_LINK_RESOLVE = 60;

// Одна запись листинга SFTP: mode/размер/время из attrs, владелец и группа — из longname
// (`ls -l`; SFTP-протокол имён не отдаёт, только uid/gid числами).
function sftpEntry(e) {
  const at = e.attrs || {};
  const mode = Number.isFinite(at.mode) ? at.mode : null;
  const ln = parseLongname(e.longname);
  const link = isLinkMode(mode) || !!(ln && ln.type === 'l');
  const dir = !link && (isDirMode(mode) || (ln ? ln.type === 'd' : /^d/.test(e.longname || '')));
  return {
    name: e.filename,
    dir,
    link,
    size: at.size || 0,
    perms: permsFromMode(mode) || (ln ? ln.perms : ''),
    mode: modeOctal(mode),
    type: fileTypeChar(mode) || (ln ? ln.type : ''),
    owner: (ln && ln.owner) || (at.uid != null ? String(at.uid) : ''),
    group: (ln && ln.group) || (at.gid != null ? String(at.gid) : ''),
    mtime: toMillis(at.mtime),
  };
}
// Симлинк в листинге показан своими атрибутами, а не целевыми: без доразрешения каталог за ссылкой
// выглядит файлом и не раскрывается. Битую ссылку помечаем broken (клик по ней объяснит причину).
async function resolveLinks(sftp, dirPath, entries) {
  const links = entries.filter((e) => e.link).slice(0, MAX_LINK_RESOLVE);
  await Promise.all(links.map(async (e) => {
    const full = posix.join(dirPath, e.name);
    e.target = await sftpReadlink(sftp, full);
    try {
      const st = await sftpStat(sftp, full);
      e.dir = typeof st.isDirectory === 'function' ? st.isDirectory() : false;
      e.targetSize = st.size || 0;
    } catch (_) { e.broken = true; }
  }));
}
...
async function fsList(c, reqPath) {
  ...
  if (b.type === 'sftp') {
    ...
    const entries = list.map(sftpEntry).filter((e) => e.name !== '.' && e.name !== '..');
    await resolveLinks(b.sftp, p, entries);
    return { ok: true, path: p, entries: sortEntries(entries) };
  }
  return ftpSerial(b, async () => {
    ...
    const entries = list.map((e) => ({
      ...
      size: e.size || 0,
      perms: permsFromFtp(e.permissions),
      mode: '',
      type: e.isDirectory ? 'd' : (e.isSymbolicLink ? 'l' : '-'),
      owner: e.user || '',
      group: e.group || '',
      mtime: toMillis(e.modifiedAt) || toMillis(e.rawModifiedAt ? Date.parse(e.rawModifiedAt) : null),
      target: e.link || '',
...
async function fsRead(c, filePath) {
  ...
  if (b.type === 'sftp') {
    let st = null;
    // нет прав на stat — читать всё равно пробуем, размер узнаем по факту чтения
    try { st = await sftpStat(b.sftp, filePath); } catch (_) { /* метаданных не будет */ }
    if (st && st.size > MAX_VIEW) return { error: 'Файл слишком большой для просмотра (> 2 МБ)', tooBig: true };
    const r = decodeForView(await sftpReadFile(b.sftp, filePath));
    if (st) {
      const mode = Number.isFinite(st.mode) ? st.mode : null;
      r.meta = { size: st.size || 0, perms: permsFromMode(mode), mode: modeOctal(mode), type: fileTypeChar(mode), owner: st.uid != null ? String(st.uid) : '', group: st.gid != null ? String(st.gid) : '', mtime: toMillis(st.mtime) };
    }
    return r;
  }
  ...
  try { await ftpSerial(b, () => b.ftp.downloadTo(sink, filePath)); } catch (e) { if (overflow) return { error: 'Файл слишком большой для просмотра (> 2 МБ)', tooBig: true }; throw e; }
  return decodeForView(Buffer.concat(chunks));
}
// Скачать файл целиком на локальный диск — без порога MAX_VIEW и без декодирования (бинарники тоже).
async function fsDownload(c, remotePath, localPath) {
  const b = await getBrowser(c);
  // Во временного соседа, потом rename: человек мог выбрать существующий файл («Заменить»), и обрыв
  // связи посреди передачи оставлял вместо него обрубок (тот же дефект чинили у скачивания из S3).
  let target = localPath;
  try { if (fs.lstatSync(localPath).isSymbolicLink()) target = fs.realpathSync(localPath); } catch (_) { /* файла нет */ }
  const tmp = path.join(path.dirname(target), '.' + path.basename(target) + '.' + process.pid + Math.random().toString(36).slice(2, 6) + '.lite-tmp');
  try {
    if (b.type === 'sftp') await sftpFastGet(b.sftp, remotePath, tmp);
    else await ftpSerial(b, () => b.ftp.downloadTo(tmp, remotePath));
    fs.renameSync(tmp, target);
  } catch (e) { try { fs.unlinkSync(tmp); } catch (_) { /* не создался */ } throw e; }
  return { ok: true, path: localPath };
}
// Запись файла на хост (save-back правок из вивера): SFTP writeFile / FTP uploadFrom.
// Записи одного файла — по очереди: два сохранения подряд (двойной Ctrl+S за время секундной записи,
// вивер и встроенный редактор над одним файлом) открывали бы его с усечением параллельно, и байты
// двух версий перемешивались. Очередь — по подключению и пути.
const writeChains = new Map();
function fsWrite(c, filePath, content) {
  const key = c.id + '\n' + filePath;
  const run = (writeChains.get(key) || Promise.resolve()).then(() => fsWriteNow(c, filePath, content));
  const tail = run.then(() => {}, () => {});
  writeChains.set(key, tail);
  tail.then(() => { if (writeChains.get(key) === tail) writeChains.delete(key); });
  return run;
}
async function fsWriteNow(c, filePath, content) {
  const b = await getBrowser(c);
  const buf = Buffer.from(String(content == null ? '' : content), 'utf8');
  // Пишем на месте, а не соседом + rename: замена меняла бы inode, и на хостах с SELinux файл получал
  // метку каталога вместо своей (/etc/crontab → etc_t — cron перестаёт его читать), рвались бы жёсткие
  // ссылки и inotify-наблюдение сервисов. Обрыв посреди записи виден: вивер оставляет правку
  // несохранённой с ошибкой, текст остаётся в буфере и локальной копии — повторное сохранение его дольёт.
  if (b.type === 'sftp') { await sftpWriteFile(b.sftp, filePath, buf); return { ok: true }; }
  const { Readable } = require('stream');
  await ftpSerial(b, () => b.ftp.uploadFrom(Readable.from(buf), filePath));
  return { ok: true };
}
  ...
  return new Promise((resolve) => {
    ...
    ssh.on('ready', () => {
      ssh.exec(String(cmd || ''), (e, stream) => {
        ...
        stream.setEncoding('utf8'); // символ, разрезанный между пакетами, не бьётся в «��»
        if (stream.stderr) stream.stderr.setEncoding('utf8');
        ...
        // code == null: сервер не прислал exit-status (считаем 0) или команду убили сигналом — это не успех
        stream.on('close', (code, signal) => fin({ ok: true, code: code != null ? code : (signal ? 128 : 0), stdout: out, stderr: err }));
      });
    });
...
const pendingTunnels = new Map(); // connId|host|port -> Promise — открытие идёт секундами, дедуп по нему
...
function openTunnelGeneric(c, label, connectStream, lport) { // lport — предпочитаемый локальный порт (занят → случайный)
  return new Promise((resolve) => {
    ...
    // тихий обрыв до готовности (только 'close', без 'error'): без этого обещание висело, а с ним и
    // pendingTunnels — все следующие открытия того же назначения ждали его до перезапуска редактора
    ssh.on('close', () => fin({ ok: false, error: 'Соединение закрыто' }));
    ...
    ssh.on('ready', () => {
      ...
      const onListen = () => {
        if (done) { try { server.close(); } catch (_) {} return; } // SSH умер, пока поднимался порт — туннель уже отвергнут
        const tunId = 'tun' + Date.now().toString(36) + Math.floor(Math.random() * 1e5).toString(36);
        t.server = server; t.port = /** @type {import('net').AddressInfo} */ (server.address()).port;
        tunnels.set(tunId, t);
      ...
      server.on('error', (e) => {
        ...
        if (lport && !retried && !done && e && /** @type {NodeJS.ErrnoException} */ (e).code === 'EADDRINUSE') { retried = true; server.listen(0, '127.0.0.1', onListen); return; }
...
// ---------------------------------------------------------------- IPC
/** @param {{ ipcMain: import('electron').IpcMain, safeStorage?: any, send?: any, getConnections?: any, setConnections?: any, onSessionOpen?: any }} deps */
function registerRemoteIpc({ ipcMain, safeStorage, send, getConnections, setConnections, onSessionOpen }) {
  ...
  ipcMain.handle('rh:save', (_e, { conn } = {}) => {
    ...
    saveConns(list);
    // Браузер файлов профиля держит соединение со СТАРЫМИ хостом/портом/паролем: после правки
    // он продолжал бы ходить туда, пока соединение не оборвётся. Следующий листинг переподключится.
    if (idx >= 0) closeBrowser(rec.id);
    return { ok: true, id: rec.id, connection: publicConn(rec) };
  });

  ipcMain.handle('rh:delete', (_e, { id } = {}) => { saveConns(loadConns().filter((x) => x.id !== id)); closeBrowser(id); return { ok: true }; });
  ...
  ipcMain.handle('rh:test', async (_e, { conn } = {}) => {
    ...
    return await new Promise((resolve) => {
      ...
      ssh.on('error', (e) => fin({ ok: false, error: String(e.message || e) }));
      ssh.on('close', () => fin({ ok: false, error: 'Соединение закрыто' })); // тихий обрыв до готовности — не висим
  ...
  ipcMain.handle('rh:open', async (e, { sessionId, id, cols, rows } = {}) => {
    ...
    // Окно модуля закрыли — интерактивная оболочка становится недостижимой (xterm умер вместе с
    // окном), но SSH-соединение и процесс на сервере жили бы до выхода из редактора. Гасим, как
    // это уже делают тейлы rmq/kafka. Туннели и SFTP-браузеры НЕ трогаем: ими пользуются
    // «Контейнеры» и вивер удалённых файлов из других окон.
    // Подписка — ДО подключения: окно, закрытое во время коннекта, уже отстреляло бы 'destroyed', и
    // подписка после него не сработала бы никогда. cleanup гасит и ещё не готовую сессию.
    const wc = e.sender;
    const onGone = () => cleanup(sessionId);
    try { wc.once('destroyed', onGone); } catch (_) {}
    const r = await openSession(sessionId, c, cols, rows);
    const unbind = () => { try { wc.removeListener('destroyed', onGone); } catch (_) {} };
    const s = sessions.get(sessionId);
    if (s) s.unbind = unbind; else unbind(); // закрытая сессия снимает слушатель — не копим их на окне
    return r;
  });
  ...
  ipcMain.handle('rh:tunnelOpen', async (_e, { id, rhost, rport, label, lport } = {}) => {
    ...
    const host = rhost || '127.0.0.1';
    for (const [tid, t] of tunnels) if (t.connId === id && t.rhost === host && t.rport === port) return { ok: true, tunId: tid, port: t.port, existed: true };
    // Дедуп выше работает только по УЖЕ поднятым туннелям, а коннект идёт секундами: двойной клик
    // по порту контейнера или по сервису хоста успевал пройти проверку дважды и поднимал два
    // туннеля на одно назначение. Второй занимал ещё один локальный порт, а вызывающий запоминал
    // только один id — закрыть лишний было уже некому. Держим обещание на время открытия.
    const key = id + '|' + host + '|' + port;
    const inflight = pendingTunnels.get(key);
    if (inflight) return inflight;
    const lp = parseInt(lport, 10);
    const p = openTunnel(c, host, port, label, (lp > 0 && lp < 65536) ? lp : undefined)
      .finally(() => pendingTunnels.delete(key));
    pendingTunnels.set(key, p);
    return p;
  });
  ...
  return {
    ...
    // «Скачать открытый файл»: main.js спрашивает локальный путь нативным диалогом и зовёт это
    downloadFile: async (id, remotePath, localPath) => { const c = findConn(id); if (!c) return { ok: false, error: 'Подключение не найдено' }; try { return await fsDownload(c, remotePath, localPath); } catch (e) { closeBrowser(id); return { ok: false, error: String(e.message || e) }; } },
