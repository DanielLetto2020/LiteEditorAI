...
async function fsRead(c, filePath) {
  ...
  try { await b.ftp.downloadTo(sink, filePath); } catch (e) { if (overflow) return { error: 'Файл слишком большой для просмотра (> 2 МБ)', tooBig: true }; throw e; }
...
async function fsDownload(c, remotePath, localPath) {
  ...
  else await b.ftp.downloadTo(localPath, remotePath);
