...
const { isReadOnlySql } = require('./sqlro');
...
async function makeHandle(c) {
  ...
  try {
    ...
    if (c.type === 'sqlite') {
      ...
      if (!SQL) SQL = initSqlJs({ locateFile: () => require.resolve('sql.js/dist/sql-wasm.wasm') }).catch((e) => { SQL = null; throw e; });
...
function registerDbIpc({ ipcMain, safeStorage, getConnections, setConnections, dialog }) {
  ...
  ipcMain.handle('db:query', async (_e, { id, sql } = {}) => {
    try {
      ...
      const fe = isReadOnlySql(sql) ? null : flushSqlite(h);
  ...
  ipcMain.handle('db:queryRo', async (_e, { id, sql, timeoutMs } = {}) => {
    try {
      if (!isReadOnlySql(sql)) return { error: 'Разрешены только читающие запросы.' };
      return await readOnlyQuery(await getHandle(id), sql, { timeoutMs });
    } catch (e) { return { error: String(e.message || e) }; }
  });
