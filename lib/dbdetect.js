...
// Спутники СУБД/брокера, а не они сами: экспортёры Prometheus (prom/mysqld-exporter,
// prometheuscommunity/postgres-exporter, kbudde/rabbitmq-exporter, danielqsj/kafka-exporter) и
// веб-интерфейсы (provectuslabs/kafka-ui, redpandadata/console, kafka-manager). Имя СУБД/брокера
// в образе есть, но подключаться к ним модулем бессмысленно, а у веб-интерфейса значок брокера
// заодно гасил «наблюдать» в мониторинге сайтов.
const SIDECAR_IMAGE = /exporter|(?:^|[/_-])(?:ui|console|manager)(?:[:@]|$)/;
const isSidecar = (image) => SIDECAR_IMAGE.test(String(image || '').toLowerCase());
...
// Адрес — из HostIp: 0.0.0.0/::/пусто — «все интерфейсы», ходим на 127.0.0.1; порт, опубликованный
// на конкретном адресе («-p 192.168.1.5:5432:5432»), на loopback не отвечает — берём этот адрес.
function hostBindFor(info, containerPort) {
  ...
  for (const src of [info.NetworkSettings && info.NetworkSettings.Ports, info.HostConfig && info.HostConfig.PortBindings]) {
    ...
    let specific = null;
    for (const b of arr) {
      const hp = parseInt(b && (b.HostPort || b.hostPort), 10);
      if (!(hp > 0)) continue;
      const ip = String((b && (b.HostIp || b.hostIp)) || '').trim();
      if (!ip || ip === '0.0.0.0' || ip === '::') return { port: hp, host: '127.0.0.1' };
      if (!specific) specific = { port: hp, host: ip };
    }
    if (specific) return specific;
  }
  return null;
}
function hostPortFor(info, containerPort) { const b = hostBindFor(info, containerPort); return b ? b.port : null; }
...
function dbPrefillFromInspect(info, engine) {
  ...
  if (isSidecar(cfg.Image || info.ImageName)) return null;   // экспортёр рядом с СУБД несёт её env, но сам не СУБД
  ...
  const bind = hostBindFor(info, DB_CONTAINER_PORT[kind]);
  const hostPort = bind ? bind.port : null;
  return {
    kind,
    ...
    prefill: {
      name, type: kind, host: bind ? bind.host : '127.0.0.1', port: hostPort || DB_CONTAINER_PORT[kind],
...
// Эвристика для списка: MinIO (S3-совместимое хранилище) — по образу; порт 9000 не детектим
// (слишком общий: php-fpm/portainer/clickhouse тоже живут на 9000).
function guessStorageKind(image) {
  return /minio/.test(String(image || '').toLowerCase()) ? 'minio' : null;
}

// Полный разбор inspect → заготовка подключения для модуля «Внешние хранилища» (адаптер S3).
// Креды из env (новые MINIO_ROOT_*, легаси MINIO_ACCESS_KEY/SECRET_KEY); API-порт MinIO — 9000.
const MINIO_API_PORT = 9000;
function storagePrefillFromInspect(info, engine) {
  if (!info || typeof info !== 'object') return null;
  const cfg = info.Config || {};
  const env = envToMap(cfg.Env);
  const is = /minio/i.test(cfg.Image || info.ImageName || '') || env.MINIO_ROOT_USER != null || env.MINIO_ACCESS_KEY != null;
  if (!is) return null;
  const name = String(info.Name || '').replace(/^\//, '') || String(info.Id || '').slice(0, 12);
  const apiBind = hostBindFor(info, MINIO_API_PORT);
  const apiPort = apiBind ? apiBind.port : null;
  return {
    kind: 'minio',
    published: apiPort != null,
    running: !!(info.State && info.State.Running),
    prefill: {
      name, adapter: 's3',
      endpoint: 'http://' + urlHost(apiBind ? apiBind.host : '127.0.0.1') + ':' + (apiPort || MINIO_API_PORT),
      region: 'us-east-1', forcePathStyle: true,
      accessKeyId: env.MINIO_ROOT_USER || env.MINIO_ACCESS_KEY || '',
      secretKey: env.MINIO_ROOT_PASSWORD != null ? env.MINIO_ROOT_PASSWORD : (env.MINIO_SECRET_KEY || ''),
      category: 'Контейнеры', source: engine + ':' + name,
    },
  };
}
...
function rmqPrefillFromInspect(info, engine) {
  if (!info || typeof info !== 'object') return null;
  const cfg = info.Config || {};
  if (isSidecar(cfg.Image || info.ImageName)) return null;
  const env = envToMap(cfg.Env);
  ...
  if (!is) return null;
  const name = String(info.Name || '').replace(/^\//, '') || String(info.Id || '').slice(0, 12);
  const mgmtBind = hostBindFor(info, RMQ_MGMT_PORT);
  const mgmtPort = mgmtBind ? mgmtBind.port : null;
  ...
  return {
    ...
    running: !!(info.State && info.State.Running),
    prefill: {
      // хост идёт в URL management API и amqp-строку (lib/rmq.js) — IPv6 в скобках
      name, host: urlHost(mgmtBind ? mgmtBind.host : '127.0.0.1'), port: mgmtPort || RMQ_MGMT_PORT, amqpPort: amqpPort || RMQ_AMQP_PORT,
...
function kafkaPrefillFromInspect(info, engine) {
  if (!info || typeof info !== 'object') return null;
  const cfg = info.Config || {};
  if (isSidecar(cfg.Image || info.ImageName)) return null;   // kafka-ui и т. п. несут адреса брокеров в env, но сами не брокеры
  const env = envToMap(cfg.Env);
...
function webPrefillFromInspect(info, engine) {
  if (!info || typeof info !== 'object') return null;
  const cfg = info.Config || {};
  let containerPort = null, hostPort = null, host = '127.0.0.1';
  for (const port of WEB_PORTS) {
    const b = hostBindFor(info, port);
    if (b) { containerPort = port; hostPort = b.port; host = urlHost(b.host); break; }
  }
  ...
  return {
    ...
    prefill: {
      name,
      url: hostPort != null ? scheme + '://' + host + ':' + hostPort : null,
      source: engine + ':' + name,
    },
  };
}

module.exports = { guessDbKind, dbPrefillFromInspect, guessMqKind, rmqPrefillFromInspect, kafkaPrefillFromInspect, guessWebKind, webPrefillFromInspect, guessStorageKind, storagePrefillFromInspect };
