...
function fetchLatest(timeoutMs = 15000) {
  return new Promise((resolve) => {
    const req = https.request(
      ...
      (res) => {
        let data = '';
        res.setEncoding('utf8');   // русские release notes: буква на стыке чанков иначе превращалась в «��» в плашке
        ...
        // Обрыв связи посреди ответа: 'end' не придёт, а у запроса 'error' тоже нет (ответ уже
        // начат) — без этого промис не разрешался никогда. После нормального 'end' — no-op.
...
function download(asset, destDir, opts = {}) {
  ...
  return new Promise((resolve) => {
    // Бросок внутри executor превратился бы в reject, а вызывающий ждёт {ok:false} — фаза
    // «загрузка» в main так и осталась бы висеть до перезапуска.
    try { fs.mkdirSync(destDir, { recursive: true }); } catch (e) { resolve({ ok: false, error: String(e.message || e), canceled: false }); return; }
    ...
    const fail = (error) => {
      try { current && current.destroy(); } catch (_) {}   // ошибка записи на диск — не качать дальше впустую
      try { out.destroy(); } catch (_) {}
      try { fs.rmSync(part, { force: true }); } catch (_) {}
      resolve({ ok: false, error, canceled });
    };
    // Слушатель сразу, а не при ответе: ошибка открытия .part приходит раньше ответа сервера и без
    // слушателя становилась бы uncaughtException, а загрузка — вечной.
    ...
    const get = (url, hops) => {
      ...
      const req = https.get(url, { headers: { 'User-Agent': UA, 'Accept': 'application/octet-stream' } }, (res) => {
        ...
        res.pipe(out);
        // Отмена (req.destroy) и обрыв связи посреди файла рвут ответ без 'end': pipe тогда не
        // закрывает .part, 'finish' не наступает, и промис не разрешался никогда — фаза в main
        // навсегда оставалась «загрузка», новую загрузку не пускало. Полный ответ — no-op.
        res.on('close', () => { if (!res.complete) fail(canceled ? 'отменено' : 'соединение прервано'); });
        out.on('finish', () => {
...
async function unpack(file, destDir, platform = process.platform) {
  ...
  // Не бросаем: вызывающий ждёт {ok:false}, а reject оставил бы фазу «распаковка» висеть в main.
  try { fs.mkdirSync(destDir, { recursive: true }); } catch (e) { return { ok: false, error: 'распаковка не удалась: ' + String(e.message || e) }; }
