...
function loadDict(code) {
  ...
  for (const f of files) {
    const d = readJson(f);
    // Только строки: значение-объект/число из пользовательского файла уходило бы из t() как есть,
    // и первая же подстановка (.replace) бросала — например, в подсказке трея.
    if (d && typeof d === 'object') for (const [k, v] of Object.entries(d)) { if (typeof v === 'string') out[k] = v; }
  }
  delete out[META];
  return out;
}
...
function setLocale(code) {
  ...
  // Код становится частью пути (locales/<код>.json). На старте он приходит из settings.lang без
  // проверки (в т.ч. из импортированного бэкапа) — '../../…' читал бы чужой JSON как словарь.
  // Допустимы ровно те имена, что available() находит в каталогах локалей.
  if (!/^[a-z][a-z0-9_-]{0,15}$/.test(current)) current = 'ru';
