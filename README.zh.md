...
*三个项目里跑着三个智能体：`web-shop` 里的已经回答，`api-gateway` 里的提了一个问题，`dashboard` 里的还在
工作。琥珀色圆点表示在等你的智能体，顶部的徽标会把它们数出来。标签页自动以任务命名；终端下方是项目文件夹、
带改动数的 git 分支和快捷栏。*

> [!NOTE]
> 界面语言可在**设置 → 外观**中切换：简体中文、English、Русский。语言文件是可插拔的
...
**尚未启动**（虚线）。项目行汇总它下面所有标签页的状态，终端顶栏还带一个计数器 —— *有多少个智能体正卡在你
身上*，点击即可跳到下一个。通知也是内置的。对 Claude Code，信号灯不靠输出去猜：状态由 Claude 自己报告，所以状态栏刷新和
打字不会触发转圈；只有在它请求权限、提问或弹出对话框时才亮琥珀色。

终端下方是当前项目的**文件夹**（菜单：打开「项目」窗口、文件管理器、复制路径）和**带改动文件数的 git
分支** —— 点击查看文件列表，点文件即可打开。动图里还能看到「更多」菜单、模块目录、外观面板和单终端模式。
[完整视频](assets/screenshots/demo.mp4)。
...
**带回滚的本地文件历史**，覆盖智能体在你提交之前所做的一切改动。代码周围还有：显示分支和 Pull / Push
提交计数的工具栏、按名称跳转文件的 `Ctrl+P`、竖排的已打开文件标签，以及显示位置、缩进、编码和换行符的状态栏。
...
**上下文**模块把项目的 `CLAUDE.md` 按章节展示在画布上：每一块以及整个文件所占的 token 量一目了然。
编辑章节会直接写入文件，旧版本进入历史记录——保留 50 份副本，可加锁防止轮换、为版本写备注、
与当前文件对比差异。智能体自己所做的修改也会被察觉并保存，因此可以像回滚自己的修改一样回滚它。

旁边还有三个标签页：**记忆**——Claude Code 记住的项目事实（事实之间的链接、与索引的差异、可恢复的
回收站）；**文件**——项目 `.claude` 与 `~/.claude` 的目录树，带编辑器、副本历史和大文件分段查看；
**对话分析**——从你与智能体的历史对话中提炼长期规则，写入 `CLAUDE.md` 或生成技能与命令的草稿。

### 4. 一切都只需点一下

侧边栏里是你的项目和三个入口：**在所有项目中查找**、**模块**和**更多**。「更多」会从侧边栏旁滑出，
里面是其余全部菜单 —— 文件夹、最近打开的项目、你自己的模块、视图与工具、帮助，以及所有内置模块。
`Ctrl+\` 会把侧边栏折叠成一条窄栏，只显示项目首字母，每个智能体的状态依然可见。

![「更多」菜单](assets/screenshots/more.png)

### 5. 服务器上的同一个项目

让智能体在你自己的服务器上继续工作，回到家从它停下的地方接着干：项目旁的云朵图标会让两台机器上的
项目文件夹保持一致。简短的设置会检查 SSH 密钥、`rsync` 和时钟，缺什么就直接给出要执行的命令。
文件只通过你自己的 `ssh` 发送到你的服务器；被替换的文件不会消失，而是进入回收站。支持 Linux 和
macOS，详见 [`scripts/server-sync`](scripts/server-sync/README.md)（英文）。
...
**一键更新。** 编辑器会在后台检查 GitHub 发布页，下载新版本，并在侧边栏底部显示 **「重启」** 提示条（版本号旁的小圆点表示有新版本）：
...
**模块**是编辑器旁边的一个独立窗口。可以从**模块**目录（带搜索的磁贴，磁贴上就能「固定到快捷栏」和
「启动时打开」）、**更多**菜单、`Ctrl+K` 或终端下方的快捷栏打开。可以同时打开多个 —— 每个窗口都记住自己的
大小和位置，下次启动时会一并恢复。绑定项目的模块会跟随当前项目。
...
| 模块 | 做什么 |
|---|---|
| 👁 **项目**（查看器 + Git） | 代码与 Git 同处一窗 —— 见[上文](#2-看清智能体改了什么)。另有 Markdown / 图片 / HTML 预览、全项目替换（`Ctrl+Shift+R`，支持正则与 `$1` 分组）、历史搜索、收藏分支、文件树内的 Git 状态。 |
| 🧠 **上下文** | 按章节展示 `CLAUDE.md`、记忆、`.claude` 文件、对话分析 —— 见[上文](#3-让智能体的上下文井井有条)。 |
...
| 🍅 **番茄钟** | 为「智能体自己在干活」的工作台准备的工作/休息计时器：休息时会有一层半透明遮罩盖住终端 —— 输入被锁住，**智能体继续运行**，输出依然可见。经典 25/5、52/17、超昼夜 90/20 或自定义方案，习惯统计带连续天数，终端顶栏有带暂停和休息操作的计时器胶囊。倒计时在后台运行。 |
| 🔊 **朗读** | 智能体的回答可以听，而不必读：选中终端输出，在右键菜单中点 **「朗读」**（或开启监听剪贴板），自然的俄语语音按句朗读——暂停、按句跳转、高亮当前句、点击某句即从该处开始读。终端杂质（边框字符、ANSI 残留、加载动画、Markdown）会被清除，数字和版本号读成文字，多音词的重音自动标注。语音引擎为 **Silero v5**，运行在独立的 Python 进程中，**不随应用分发**（CC BY-NC-SA 许可）：需要装有 `torch` 的 Python，模型（145 MB）一键下载。仅支持俄语。 |
...
**🧩 自己写模块。** *模块 → 我的模块 → 创建模块* 会生成一个插件骨架，并**在它的目录里打开一个终端**，
**你自己的 AI 智能体**就在那里按随附的规范写代码（`GUIDE.md` 和提示词会一起放在旁边）。
写好的模块出现在「我的模块」下，可热重载。作者规范见 [`module-kit/`](module-kit/)。

## 外观

只有一套主题 —— **石墨（Graphite）**：深色窗口上的圆角卡片，窗口背景和终端略微透明，菜单和按钮保持不透明。
颜色由你决定：强调色、六种基础颜色、状态颜色，以及另外 16 种可手动设置的颜色，还有圆角、侧边栏宽度、项目行
高度、终端字体和背景不透明度。这些都在**外观**面板（侧边栏底部的调色板按钮）里实时生效，模块窗口也会同步
换色（包括透明度）；主题还可以作为文本复制和粘贴，方便分享。

![外观面板](assets/screenshots/appearance.png)
...
| `Ctrl+\` | 单终端模式 |
| `Ctrl+K` | 命令面板 |
| `Ctrl+F` / `Ctrl+Shift+F` | 在终端或文件中搜索 / 在所有项目中查找（文件与终端输出） |
| `Ctrl+S` | 保存文件 |
...
（进程本身不能；不过进程能挺过窗口重新加载，窗口崩溃也不会让智能体停下）。查看器以竖排标签列出已打开的文件，跳过大于 2 MB 的文件。它是一个「能编辑的查看器」，
不是要替代你 IDE 的重构引擎 —— 这是刻意的取舍。

**v1.1.173 之后已移除：Android 遥控端及其中继服务器。** `.apk`、电脑端的中继客户端以及 `relay/` 目录下的
自建中继都已从项目中删除。最后一个包含它们的发行版是
[v1.1.173](https://github.com/DanielLetto2020/LiteEditorAI/releases/tag/v1.1.173)：可以从那里下载，
或按该标签恢复源码。
