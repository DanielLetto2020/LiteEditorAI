...
| 🔌 **远程主机** | 按分类管理的 **SSH / SFTP / FTP** 配置，一键登录、多个活动会话以标签页并存（密码或系统密钥、keepalive）、SFTP/FTP 文件浏览、**在查看器中编辑远程文件** —— 每次保存都会传回主机。**「服务」**按钮会扫描主机端口，并**通过 SSH 隧道**在各自模块里打开 Postgres / MySQL / RabbitMQ / Kafka —— 即使服务只监听主机的 localhost 也可以；Web 端口交给站点监控，docker/podman socket 交给容器模块。密码永不离开后端。 |
