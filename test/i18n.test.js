...
// --- Код языка не выводит путь за каталоги локалей ------------------------------------
// settings.lang на старте идёт в setLocale без проверки (в т.ч. из импортированного бэкапа).
fs.writeFileSync(path.join(HOME, 'secret.json'), JSON.stringify({ 'Сохранить': 'LEAKED' }), 'utf8');
eq(i18n.setLocale('../../secret'), 'ru', 'код с «../» отвергнут — русский');
eq(i18n.t('Сохранить'), 'Сохранить', 'чужой JSON словарём не стал');
eq(i18n.setLocale({ x: 1 }), 'ru', 'не-строка вместо кода — русский');

// --- Не-строковые значения в словаре не ломают t() --------------------------------------
fs.writeFileSync(path.join(USER_LOCALES, 'ww.json'), JSON.stringify({
  'Сохранить': { one: 'x' }, 'Найдено {0} файлов': 5, 'Открыть': 'WW-open',
}), 'utf8');
i18n.setLocale('ww');
eq(typeof i18n.t('Сохранить'), 'string', 'значение-объект пропущено, t() отдаёт строку');
eq(typeof i18n.t('Найдено {0} файлов', 3), 'string', 'значение-число в шаблоне не роняет подстановку');
eq(i18n.t('Открыть'), 'WW-open', 'строковые значения того же файла работают');
